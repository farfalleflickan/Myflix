...
if [[ ! -z $TMDBapi ]]; then
	...
	output=$(curl -s --request GET --url $myUrl --data '{}' | jq -r 'if has("posters") then .posters | map(select((.width | contains(680)) and (.height | contains(1000))) | .file_path) else "null" end' | head -n1)
	if [[ $output == *".jpg"* ]]; then
