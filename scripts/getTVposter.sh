...
myID=${1}
if [[ ! -z $TMDBapi ]]; then
	myUrl="https://api.themoviedb.org/3/tv/"${myID}"/images?include_image_language=en&api_key="${TMDBapi}
	output=$(curl -s --request GET --url $myUrl --data '{}' | jq -r 'if has("posters") then .posters | map(select((.width | contains(680)) and (.height | contains(1000))) | .file_path ) else "null" end')
	if [[ $output == *".jpg"* ]]; then
